...
void DetectionAlgorithm::detectMarkers(std::shared_ptr<Deque> deque){
    ...
    for (int i = int(deque->size()) - 1; i >= 0; i--) {
        for (const Metavision::Event2d* ev : *deque->at(i)->first) {
            if (ev->x >= 0 && ev->x < camera_width && ev->y >= 0 && ev->y < camera_height){
            ...
            if(ev->t < finish_ts){
                ...
                for (unsigned int j = 0; j < boundRect.size(); j++) {
                    ...
                    for (int x = std::max(0, boundRect.at(j).x); x <= std::min(boundRect.at(j).x + boundRect.at(j).width, camera_width - 1); ++x) {
                        for (int y = std::max(0, boundRect.at(j).y); y <= std::min(boundRect.at(j).y + boundRect.at(j).height, camera_height - 1); ++y) {
                            int hist_value = (int)histogram.at<uint8_t>(y, x);
                            for (int channel = 0; channel < std::max(hist_value - 1, 0); ++channel) {
                                // gathered_timestamps 保护
                                if (channel + 1 < gathered_timestamps.size[2]) {
                                    diffs[ std::min( std::abs(gathered_timestamps.at<int>(y, x, channel + 1)
                                        - gathered_timestamps.at<int>(y, x, channel)), 200) ] += 1;
                                }
                            }
                        }
                    }
