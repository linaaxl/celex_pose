...
int main(int argc, char *argv[]) {
    ...
    try { // 开始 try 块
        #ifdef USE_CELEX
            CeleXCamera cam(&parser.current_setup.getCamConfig());
            ...
            runtime.buffers.setInputBuffer(cam.reader_buffers.getOutputBuffer());

            VisualizationController vis(cam.width, cam.height, &cam, &runtime);
            vis.buffers.setInputBuffer(cam.reader_buffers.getOutputBuffer());
            ...
            vis.start();
            runtime.start();
            // CeleX数据流回调已经自动塞到缓冲区，不需要额外reader.start()
        #else
