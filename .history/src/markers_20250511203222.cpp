...
void Marker::track() {
    ...
    // 在 Marker::solvePnP() 和 Marker::track() 中调用 solvePnP 之前
    ...
    bool status = cv::solvePnP(objectpoints_3d, detected_initial_points, this->cam_obj->camera_matrix_cv, this->cam_obj->dist_coeffs,r_vec, t_vec, false, cv::SOLVEPNP_IPPE_SQUARE);
    if (!status){
        stopTracking();
    ...
    // cv::projectPoints(center_3d, r_vec, t_vec, cam_obj->camera_matrix_cv, cam_obj->dist_coeffs, projected_tmp);
...
void Marker::solvePnP() {
    ...
    // 在 Marker::solvePnP() 和 Marker::track() 中调用 solvePnP 之前
    if (logger_ptr) {
        logger_ptr->log(Utils::LOG_LEVELS::DEBUG, "[Marker ID: " + std::to_string(id) +
                                                "] Attempting PnP. Object points: " + std::to_string(objectpoints_3d.size()) +
                                                ", Image points: " + std::to_string(blob_centers.size())); // 或 detected_initial_points.size()
    } else {
        std::cout << "[Marker ID: " << id << "] Attempting PnP. Object points: " << objectpoints_3d.size()
                << ", Image points: " << blob_centers.size() << std::endl;
    }

    if (objectpoints_3d.size() != blob_centers.size() || objectpoints_3d.size() < 3) { // SQPNP 至少需要3个点，通常4个或更多效果更好
        if (logger_ptr) {
            logger_ptr->log(Utils::LOG_LEVELS::WARNING, "[Marker ID: " + std::to_string(id) +
                                                        "] Insufficient or mismatched points for PnP. Skipping.");
        } else {
            std::cerr << "[Marker ID: " << id << "] Insufficient or mismatched points for PnP. Skipping." << std::endl;
        }
        // 根据情况设置 tracking_lost = true; 或其他错误处理
        if (pnp_running.load()) pnp_running = false; // 确保重置标志
        return; // 在 solvePnP() 中
        // 在 track() 的初始PnP中，可能调用 stopTracking(); markers_manager_ptr->deregisterMarker(this); return;
    }
