...
void Marker::solvePnP() {
    ...
    logger_ptr->log(Utils::LOG_LEVELS::DEBUG, "[Marker ID: " + std::to_string(id) +
                                            " SOLVE_PNP_LOOP] Attempting PnP. Object points: " + std::to_string(objectpoints_3d.size()) +
                                            ", Image points (blob_centers): " + std::to_string(blob_centers.size()));
    } else {
        std::cout << "[Marker ID: " << id << " SOLVE_PNP_LOOP] Attempting PnP. Object points: " << objectpoints_3d.size()
                << ", Image points (blob_centers): " << blob_centers.size() << std::endl << std::flush;
    }
    ...
    if (objectpoints_3d.size() != blob_centers.size() || objectpoints_3d.size() < 3) {
        if (logger_ptr) {
            logger_ptr->log(Utils::LOG_LEVELS::WARNING, "[Marker ID: " + std::to_string(id) +
                                                        " SOLVE_PNP_LOOP] Insufficient or mismatched points for PnP. Skipping.");
        } else {
            std::cerr << "[Marker ID: " << id << " SOLVE_PNP_LOOP] Insufficient or mismatched points for PnP. Skipping." << std::endl << std::flush;
        }
        tracking_lost = true; // 根据您的逻辑
        // pnp_running = false; // 应该在调用此函数的地方（track循环中）的线程函数返回后重置，或者在 PnP 线程结束时重置
