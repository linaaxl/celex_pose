...
void Marker::track() {
    ...
    bool status = cv::solvePnP(objectpoints_3d, detected_initial_points, this->cam_obj->camera_matrix_cv, cam_obj->dist_coeffs,r_vec, t_vec, true, cv::SOLVEPNP_IPPE_SQUARE);
    if (!status){
        stopTracking();
