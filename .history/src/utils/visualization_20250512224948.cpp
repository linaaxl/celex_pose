...
void VisualizationController::runtimeLoop() {
    ...
    closed = true;
        ...
        std::cerr << "[VIS_LOOP_THREAD] OpenCV Exception: " << cv_e.what()
                  << " (File: " << cv_e.file << ", Line: " << cv_e.line << ", Func: " << cv_e.func << ")" << std::endl << std::flush;
        to_close = true; // 尝试触发关闭
    } catch (const std::exception& e) {
        std::cerr << "[VIS_LOOP_THREAD] Std Exception: " << e.what() << std::endl << std::flush;
        to_close = true; // 尝试触发关闭
    } catch (...) {
        std::cerr << "[VIS_LOOP_THREAD] Unknown exception." << std::endl << std::flush;
        to_close = true; // 尝试触发关闭
    }
    // 无论如何，runtimeLoop 结束时都应该尝试设置 closed
    if (ui_ptr) { ui_ptr->set_close_flag(); }
    std::this_thread::sleep_for(std::chrono::microseconds(500));
    closed = true;
    std::cout << "[VIS_LOOP_THREAD] 'closed' flag set to true." << std::endl << std::flush;
